...
class SpaldRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'skala'                 => ['required', Rule::in(SkalaPelayanan::cases())],
            'tahun_konstruksi'      => 'required|date_format:Y',
            'sumber'                => ['required', Rule::in(SumberDana::cases())],
            'status_keberfungsian'  => ['required', Rule::in(OpsiBerfungsi::cases())],
            'kondisi'               => ['required', Rule::in(OpsiBaik::cases())],
            'status_lahan'          => ['required', Rule::in(StatusLahan::cases())],
            'kapasitas'             => 'nullable|numeric|gte:0',
            'jenis'                 => ['required', Rule::in(JenisPengelolaan::cases())],
            'teknologi'             => ['required', Rule::in(OpsiTeknologi::cases())],
            ...
            'status_penyedotan'     => ['required', Rule::in(OpsiAda::cases())],
